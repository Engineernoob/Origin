...
export class TrainingDataService {
  ...
  async collectTrainingData(
    ...
    this.logger.log(
      `Training data collection completed. Total examples: ${collectedCount}`,
    );
  }

  private async collectFromQuery(
    query: string,
    maxResults: number,
  ): Promise<void> {
  ...
  private calculateLabels(video: YouTubeVideo, isTrending = false): any {
    ...
    const viralScore =
      Math.min(1, viewVelocity / 10000) * (isTrending ? 1.5 : 1);
  ...
  private async saveTrainingExample(example: TrainingExample): Promise<void> {
    try {
      ...
      this.logger.error(
        `Error saving training example for video ${example.features.videoId}:`,
        error,
      );
    }
  }
