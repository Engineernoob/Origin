...
export class VideoProcessingService {
  ...
  private async generateQuality(videoId: string, inputPath: string, quality: string): Promise<void> {
    return new Promise((resolve, reject) => {
      const outputPath = join(this.outputPath, `${videoId}-${quality}.mp4`);
      const { width, height, videoBitrate, audioBitrate } = this.getQualitySettings(quality);
      ...
      ffmpeg(inputPath)
        ...
        .on('start', (commandLine) => {
          this.logger.log(`FFmpeg process started for ${quality}: ${commandLine}`);
        })
        .on('progress', (progress) => {
          this.logger.log(`Processing ${quality}: ${Math.round(progress.percent || 0)}% done`);
        })
        .on('end', () => {
  ...
  private async generateThumbnail(videoId: string, inputPath: string): Promise<void> {
    return new Promise((resolve, reject) => {
  ...
  private async extractVideoMetadata(videoId: string, inputPath: string): Promise<any> {
    return new Promise((resolve, reject) => {
      ffmpeg.ffprobe(inputPath, (err, metadata) => {
        ...
        this.logger.log(`Extracted metadata for video ${videoId}:`, videoMetadata);
  ...
  private getQualitySettings(quality: string) {
    const settings = {
      ...
      '1080p': { width: 1920, height: 1080, videoBitrate: '5000k', audioBitrate: '256k' },
      '1440p': { width: 2560, height: 1440, videoBitrate: '10000k', audioBitrate: '256k' },
      '2160p': { width: 3840, height: 2160, videoBitrate: '25000k', audioBitrate: '320k' },
    };
